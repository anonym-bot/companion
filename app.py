...
def initial(user_id, query, mode, edit_id, format):
    ...
    if format == 'T':
        ...
        if tts(output):
            ...
            reply_markup = {'inline_keyboard': [[{'text': f"Regenerate ♻️", 'callback_data': f'R {mode}'},{'text': "Try different AI ⏭", 'callback_data': f"A {mode}"}],[{'text': f"Draft 1", 'callback_data': f'D {copy_id} 1'}]]}
            print(requests.post(f'https://api.telegram.org/bot{BOT_TOKEN}/sendVoice', params={'chat_id': user_id, 'caption': 'Voice message caption', 'reply_markup':reply_markup}, files={'voice': open('random.ogg', 'rb')}).json())
